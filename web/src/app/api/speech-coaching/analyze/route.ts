...
/** 2 min mono PCM16 @ 48kHz ≈ 11.5MB + WAV header — leave headroom */
...
function parseWavFromRequest(request: Request, buf: ArrayBuffer): Buffer | null {
    ...
    try {
        const json = JSON.parse(new TextDecoder().decode(buf)) as { audio_base64?: string };
...
export async function POST(request: Request) {
    ...
    if (!apiKey) {
        return Response.json({ status: "error", message: "OPENROUTER_API_KEY is not configured" }, { status: 500 });
    }
    ...
    if (ct.includes("multipart/form-data")) {
        ...
        if (ab.byteLength > MAX_BODY_BYTES) {
            return Response.json({ status: "error", message: "Audio too large" }, { status: 413 });
        }
        wav = Buffer.from(ab);
    } else {
        ...
        if (!parsed) {
            return Response.json(
                { status: "error", message: "Use multipart/form-data with audio field, or JSON { audio_base64 }" },
                { status: 400 },
            );
        }
        wav = parsed;
